# [[Comet_Tail_Sign|Comet Tail Sign]]

## Definition
Eine subpleurale weichteildichte Raumforderung (Rundatelekase), von der Bronchovaskulärbündel wie ein Kometenschweif bogenförmig in Richtung Hilus ziehen.

## Radiologisches Erscheinungsbild

> 🖼️ **Fleischner Referenzbild (Glossary):**
> ![Comet Tail Sign](../../../media/comet_tail_sign_fleischner.png)
> *(Bitte entsprechendes Bild aus dem Fleischner-PDF hierher ziehen und als `comet_tail_sign_fleischner.png` im Ordner `media/` speichern)*

- Typisches Zeichen in der Thorax-Bildgebung (Röntgen oder HRCT).
- Dient der Eingrenzung spezifischer, oft pathognomonischer Differenzialdiagnosen (Fleischner Glossary Criteria).

## Ätiologie / Differenzialdiagnosen
- Rundatelektase (Blesovsky-Syndrom) - assoziiert mit Asbestose oder alten Pleuraergüssen/Pleuritis.

## Weiterführende Links
- [Radiopaedia: Comet Tail Sign](https://radiopaedia.org/articles/comet-tail-sign-thorax)

---
Tags: #Radiologie #Lunge #Zeichen #Sign #Fleischner

## Video-Empfehlung
- [Kurzes Erklärvideo auf YouTube ansehen](https://www.youtube.com/watch?v=TzSax6EOckc)

